...
int main() {
    ...
    std::queue<Process*> high_priority_queue;
    std::queue<Process*> low_priority_queue;
    ...
    while (completed_processes < processes.size()) {
        while (process_idx < processes.size() && processes[process_idx].arrival_time <= current_time) {
            if (processes[process_idx].priority < 3) {
                high_priority_queue.push(&processes[process_idx]);
            } else {
                low_priority_queue.push(&processes[process_idx]);
            }
            process_idx++;
        }
        ...
        if (!high_priority_queue.empty()) {
            ...
            if (current_process->remaining_time > 0) {
                high_priority_queue.push(current_process);
            } else {
            ...
            int time_to_run = current_process->remaining_time;
            gantt.push_back({current_process->id, time_to_run});
